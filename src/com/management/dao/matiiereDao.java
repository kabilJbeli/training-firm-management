package com.management.dao;

import java.util.ArrayList;
import java.util.List;

import javax.persistence.EntityTransaction;
import javax.persistence.Query;

import com.managament.entities.Matiere;

public class matiiereDao implements matiereInterface {

	private entityManagerConexion em = new entityManagerConexion();

	public Matiere add(Matiere form) {
		// TODO Auto-generated method stub
		// TODO Auto-generated method stub
		Matiere f = new Matiere();
		try {
			EntityTransaction tx = em.getEntityManager().getTransaction();
			tx.begin();
			em.getEntityManager().persist(form);
			tx.commit();
			f = em.getEntityManager().find(Matiere.class, form.getId());

		} catch (IllegalStateException e) {
			em.getEntityManager().getTransaction().rollback();
			em.getEntityManager().close();
		}
		return f;
	}

	public void remove(int code) {
		// TODO Auto-generated method stub

		EntityTransaction tx = em.getEntityManager().getTransaction();
		try {
			tx.begin();
			Matiere f = em.getEntityManager().find(Matiere.class, code);
			if(f != null) {
		    em.getEntityManager().remove(f);
			}else {
				System.out.println("The requested Don entity with the following ID "+code+" Doesn't exist in the database");
			}
			tx.commit();

		} catch (Exception e) {
			em.getEntityManager().getTransaction().rollback();
			em.getEntityManager().close();
		}
	}

	public List<Matiere> findAll() {
		// TODO Auto-generated method stub
		// TODO Auto-generated method stub
		EntityTransaction tx = em.getEntityManager().getTransaction();
		List<Matiere> Forms = new ArrayList<Matiere>();

		try {
			tx.begin();
			Query query = em.getEntityManager().createNativeQuery("SELECT * FROM `center-test`.matiere;");
			Forms = query.getResultList();
			tx.commit();
		} catch (Exception e) {
			em.getEntityManager().getTransaction().rollback();
			em.getEntityManager().close();
		}
		return Forms;
	}

	public Matiere find(int code) {
		// TODO Auto-generated method stub
		EntityTransaction tx = em.getEntityManager().getTransaction();
		Matiere f = new Matiere();
		try {
			tx.begin();
			f = em.getEntityManager().find(Matiere.class, code);
			tx.commit();
		} catch (Exception e) {
			em.getEntityManager().getTransaction().rollback();
			em.getEntityManager().close();
		}
		return f;
	}

	public boolean update(Matiere form) {
		// TODO Auto-generated method stub
		EntityTransaction tx = em.getEntityManager().getTransaction();
		Boolean isupdated = false;
		try {
			tx.begin();
			form = em.getEntityManager().merge(form);
			Matiere searchedForm = em.getEntityManager().find(Matiere.class, form.getId());
			tx.commit();
			isupdated = form.equals(searchedForm);
		} catch (Exception e) {
			em.getEntityManager().getTransaction().rollback();
			em.getEntityManager().close();
		}
		return isupdated;
	}

}
