AppModule.directive('allFormation', function() {
    return {
        ...
        controller: function($scope, $log,$http) {
            ...
            $scope.getAllFormation = function(){            	
             ...
             $scope.getAllFormation();
                ...
                $http.post("/training-firm-management/api/dons/add/formation/"+name).then(function(response) {
                	if(response.data !== ""){
                    $scope.addingItemError=false;
       ...
       $scope.updateSelectedItem = function(name) {
       console.log(name);
            	$scope.dataFormation.name = name;
                $http.put("/training-firm-management/api/dons/modifyFormation?id="+$scope.dataFormation.id+"&name="+name).then(function(response) {
                    console.log(response);
                    $scope.getAllFormation();
